...
from utils.dataset_RGB import GTRainDataset, CustomBatchSampler
...
#             target = data['input_img'].cuda()
#             input_ = data['target_img'].cuda()
...
for epoch in range(start_epoch, opt['TRAIN']['TOTAL_EPOCHS'] + 1):
    ...
    for iter, data in enumerate(train_loader, 1):
        ...
        if epoch >=  opt['TRAIN']['SAVE']:
            if iter % opt['TRAIN']['SAVE_FRE_ITERATION'] == 0:
                torch.save({'epoch': epoch, 
                            'state_dict': model_restoration.state_dict(),
                            'optimizer' : optimizer.state_dict()
                            }, os.path.join(model_dir,f"big_image_model_epoch_{epoch}_iter_{iter}.pth")) 
       
            
    #### Save ####
    if epoch >=  opt['TRAIN']['SAVE']:
        if epoch % opt['TRAIN']['SAVE_FRE'] == 0 or epoch>= 20:
            torch.save({'epoch': epoch, 
                        'state_dict': model_restoration.state_dict(),
                        'optimizer' : optimizer.state_dict()
    ...
    if epoch >= opt['TRAIN']['VAL']:
        if epoch % opt['TRAIN']['VAL_FRE'] == 0:
            ...
            for ii, data_val in enumerate(tqdm(val_loader), 1):
                # print("data_val['target_img'].shape = ", data_val['target_img'].shape)
                target_img = img_as_ubyte(data_val['target_img'].numpy().squeeze().transpose((1,2,0)))
                input_ = data_val['input_img'].cuda()
                # factor = 8
                ...
                ssim_val_rgb.append(ssim(restored_img, target_img, data_range=255, channel_axis=-1))
                log('%-6s \t %f \t %f' % (data_val['file_name'][0], psnr_val_rgb[-1], ssim_val_rgb[-1]), os.path.join(log_dir, 'val_' + str(epoch)+'.txt'), P=False)
                if opt['TRAIN']['SAVE_IMG'] and epoch >= opt['TRAIN']['START_IMG']:
                    save_img_path = os.path.join(result_dir, 'epoch_' + str(epoch))
                    file_name = data_val['file_name'][0] + '_restored.png'
            ...
            log('total images = %d \t avg_psnr = %f \t avg_ssim = %f' % (ii,avg_psnr,avg_ssim), os.path.join(log_dir, 'val_' + str(epoch)+'.txt'), P=False)
            ...
            log("[epoch %d PSNR: %.4f SSIM: %.6f --- best_epoch %d Best_PSNR %.4f]" % (epoch, avg_psnr, avg_ssim, best_epoch, best_psnr),train_log)
